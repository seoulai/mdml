...
DROP MATERIALIZED VIEW IF EXISTS gcsdaily CASCADE;
create materialized view gcsdaily as
with base as
(
...
, gcs_final as (
  select gcs.*, date_trunc('day', gcs.charttime) as charttime_by_day
  -- This sorts the data by GCS, so rn=1 is the the lowest GCS values to keep
  , ROW_NUMBER ()
          OVER (PARTITION BY date_trunc('day', gcs.charttime)
                ORDER BY gcs.GCS
               ) as IsMinGCS
  from gcs
)
select ie.SUBJECT_ID, ie.HADM_ID, ie.ICUSTAY_ID, gs.charttime_by_day
...
ORDER BY ie.ICUSTAY_ID, gs.charttime_by_day;
