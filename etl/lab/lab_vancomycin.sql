...
SELECT st1. subject_id
       , st1.hadm_id AS hadm_id
       , st1.icustay_id AS icustay_id
       , st1.time_id AS time_id
       , time_id::timestamp - LAG( st1.time_id, 1) OVER (ORDER BY st1.time_id)::timestamp AS timestamp_diff
       , st1.date_id AS date_id
       , st1.date_id - LAG( st1.date_id, 1) OVER (ORDER BY st1.time_id) AS date_diff
       --, st1.lab_cnt AS lab_cnt
       --, st1.normal_cnt AS normal_cnt
       --, st1.abnormal_cnt AS abnormal_cnt
       --, st1.delta_cnt AS delta_cnt
       , st1.item_info AS item_info
FROM lab_statistics_41976 st1
;
