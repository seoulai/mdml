DROP MATERIALIZED VIEW IF EXISTS gcshours CASCADE;
create materialized view gcshours as
with base as
(
...
, gcs as (
  ...
  from base b
  -- join to itself within 1 hours to get previous value
  left join base b2
    on b.ICUSTAY_ID = b2.ICUSTAY_ID and b.rn = b2.rn+1 and b2.charttime > b.charttime - interval '1' hour
)
, gcs_final as (
  select gcs.*, date_trunc('hour', gcs.charttime) as charttime_by_hour
  -- This sorts the data by GCS, so rn=1 is the the lowest GCS values to keep
  , ROW_NUMBER ()
          OVER (PARTITION BY date_trunc('hour', gcs.charttime)
                ORDER BY gcs.GCS
               ) as IsMinGCS
  from gcs
)
select ie.SUBJECT_ID, ie.HADM_ID, ie.ICUSTAY_ID, gs.charttime_by_hour
...
ORDER BY ie.ICUSTAY_ID, gs.charttime_by_hour;
